...
├── data_loader.py            # Data loading, pre-computed rank changes, commodity edge filtering
├── components/
│   ├── __init__.py
...
├── callbacks/
│   ├── __init__.py
│   └── interactions.py       # All Dash callbacks (commodity-aware edge rendering)
├── styles/
│   ├── __init__.py
│   └── css.py                # Custom CSS
├── data/
...
│   ├── commodity_edges.csv      # Per-commodity edge weights (80k edges, 50 SCTG codes)
...
- **Commodity filtering**: Filter both node centralities AND edge flows by SCTG commodity code (42 individual + 8 grouped codes)
...
- Commodity edges: Extracted from CFS 2017 via thesis pipeline's `build_commodity_network()`
  - 80,867 edges across 50 SCTG codes (42 individual + 8 grouped)
  - Generated by `active/thesis/extract_commodity_edges.py`
  - SCTG 16 (Crude Petroleum) has no CFS data (pipeline-transported)
